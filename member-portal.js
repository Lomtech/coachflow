...
async function loadCoachData() {
  ...
  coach = coachData;
  document.getElementById("coachNameTitle").textContent =
    coach.name + " - Membership";
...
async function subscribe() {
  ...
  try {
    const response = await fetch(
      "/.netlify/functions/create-customer-checkout",
      {
        method: "POST",
        headers: {
...
async function loadContent() {
  ...
  filtered.forEach((item) => {
    ...
    const icon =
      currentTab === "videos" ? "🎥" : currentTab === "documents" ? "📄" : "🖼️";
